...
// User Auth & Cloud SQL Synchronization Endpoint
app.post("/api/auth/sync", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const bodyDisplayName = req.body.displayName;
    const bodyPhotoUrl = req.body.photoUrl;

    const user = await getOrCreateUser(
      uid,
      email || req.body.email || `${uid}@triptale.app`,
      bodyDisplayName || name || null,
      bodyPhotoUrl || picture || null,
      email_verified || req.body.emailVerified
    );
...
// Direct Cloud SQL Email & Password Registration Endpoint
...
// Direct Cloud SQL Email & Password Login Endpoint
...
app.post("/api/auth/verify-email", requireAuth, async (req: AuthRequest, res) => {
  try {
    if (!req.user?.uid) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    const updated = await verifyUserEmailByUid(req.user.uid);
...
// Authenticated Cloud SQL Trips Endpoints
app.get("/api/trips", requireAuth, async (req: AuthRequest, res) => {
  try {
    if (!req.user?.uid) {
      return res.status(401).json({ error: "Unauthorized" });
    }

    const dbTrips = await getUserTrips(req.user.uid);
    res.json({ success: true, trips: dbTrips });
  } catch (error: any) {
...
app.post("/api/trips", requireAuth, async (req: AuthRequest, res) => {
  try {
    if (!req.user?.uid) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    if (!id || !customName || !startLocation || !destination) {
      return res.status(400).json({ error: "Missing required trip fields" });
    }

    const saved = await saveUserTrip({
      id,
      userId: req.user.uid,
      customName,
      startLocation,
      ...
      totalPlannedBudget,
      transportMode,
      planData: typeof planData === "object" ? JSON.stringify(planData) : planData,
      notes,
    });
...
app.delete("/api/trips/:id", requireAuth, async (req: AuthRequest, res) => {
  try {
    if (!req.user?.uid) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    await deleteUserTrip(tripId, req.user.uid);
